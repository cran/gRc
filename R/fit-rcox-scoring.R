...
## FIXME: Either check on input or create rcox-methd
## There is only one scoring function and it branches on class rcon /
## rcor in four places.
...
scoring <- function(object, K0, 
  ...
  theta0      <- K2theta(object, K0, scale='free')  ## branch on class
  curr.logL   <- logL0
  curr.theta  <- theta0
  ...
  repeat {
    itcount <- itcount + 1
    xxx     <- getScore(object, curr.K, scale='free') ## branch on class
    ...
    repeat{
      ...
      new.K       <- theta2K(object, new.theta, scale='free') ## branch on class
  ...
  J       <- getScore(object, curr.K, scale='original')$J ## branch on class
